...
static  int query_result_parse(pg_object *pg_object)
{
    ...
    switch (status) {
        ...
        case PGRES_FATAL_ERROR:
            ...
            zend_update_property_string(swoole_postgresql_coro_class_entry_ptr, pg_object->object, "error", 5, errMsg);
...
//    swoole_postgresql_coro_close(pg_object);
    ...
    zend_update_property_string(swoole_postgresql_coro_class_entry_ptr, pg_object->object, "error", 5, "onerror");
...
static int swoole_postgresql_coro_close(pg_object *pg_object)
{
    ...
    if (_socket->active == 1) {
        PGresult *res;
        while ((res = PQgetResult(pg_object->conn)))
        {
            PQclear(res);
        }
        PQfinish(pg_object->conn);
    }
